                            ...
                            cv::Point2f &best_match_norm)
{
    ...
    int bestDist = 128;
    int bestIndex = -1;
	//printf("descriptors_old size is %d", (int)descriptors_old.size());
...
void KeyFrame::searchByBRIEFDes(std::vector<cv::Point2f> &matched_2d_old,
                                ...
                                const std::vector<cv::KeyPoint> &keypoints_old_norm)
{
	//printf("searchByBRIEFDes,window_brief_descriotors size is %d \n", (int)window_brief_descriptors.size());
...
bool KeyFrame::findConnection(KeyFrame* old_kf)
{
	...
	matched_3d = point_3d;
	//printf("cur-keyframe point_3d size: %lu \n ", point_3d.size());
	/*
	printf("old-keyframe point_3d size: %lu \n ", old_kf->point_3d.size());
	printf("cur-keyframe point_rgbd size: %lu \n ", point_rgbd.size());
	printf("old-keyframe point_rgbd size: %lu \n ", old_kf->point_rgbd.size());
	
	*/
	matched_2d_cur = point_2d_uv;
	//printf("cur-keyframe point_2d_uv size: %lu \n" ,point_2d_uv.size());
	//printf("old-keyframe point_2d_uv size: %lu \n", old_kf->point_2d_uv.size());
	matched_2d_cur_norm = point_2d_norm;
	//printf("cur-keyframe point_2d_norm size: %lu \n" ,point_2d_norm.size());
	matched_id = point_id;
	//printf("cur-keyframe point_id size: %lu \n", point_id.size());
	...
	*/
	...
	double relative_yaw;
	//printf("find connect 494 \n");
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
		printf("matched_2d points %d is more than MIN_Loop_num %d \n",(int)matched_2d_cur.size(),MIN_LOOP_NUM);
		status.clear();
	    ...
	    reduceVector(matched_id, status);
	    	...
	    	if (DEBUG_IMAGE)
	        {
	            ...
	            cv::hconcat(gap_image, old_img, gray_img);
	            cvtColor(gray_img, loop_match_img, CV_GRAY2RGB);
				printf("size of matched_2d_cur is %d \n", (int)matched_2d_cur.size());
	            for(int i = 0; i< (int)matched_2d_cur.size(); i++)
	            {
	                cv::Point2f cur_pt = matched_2d_cur[i];
	                cv::circle(loop_match_img, cur_pt, 5, cv::Scalar(0, 255, 0));
	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
		//PnP_R_old 回环帧在世界坐标系下位姿

	    relative_t = PnP_R_old.transpose() * (origin_vio_T - PnP_T_old);
		//only modifiy x and y,2021-9-2 hmh
		relative_t(2) = 0;
		
	    relative_q = PnP_R_old.transpose() * origin_vio_R;
	    relative_yaw = Utility::normalizeAngle(Utility::R2ypr(origin_vio_R).x() - Utility::R2ypr(PnP_R_old).x());
		/*
		Eigen::Matrix3d relative_R_yaw;
		relative_R_yaw = Utility::ypr2R(Eigen::Vector3d(relative_yaw,0,0));
		Quaterniond relative_q_yaw(relative_R_yaw);
		*/
		//printf("PNP relative\n");
	...
	//printf("---------loop keypoint %d,find connect use time: %lf--------------- \n", (int)matched_2d_cur.size(), t_match.toc());
	return false;
}
