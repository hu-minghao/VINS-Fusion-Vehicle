...
map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> FeatureTracker::trackImage(double _cur_time, const cv::Mat &_img, const cv::Mat &_img1)
{
    ...
    if (prev_pts.size() > 0)
    {
        ...
        if(hasPrediction)
        {
            ...
            //printf("has prediction, the number of track success is %d \n",succ_num);
            if (succ_num < 10)
        ...
        else
        {
            ...
            //printf("has no prediction, the number of track success is %d \n", succ_num);
        }
            // reverse check
        if(FLOW_BACK)
        {
        ...
        printf("finally, the number of track success is %d \n", succ_num_new);
        */
    ...
    //追踪次数加1 hmh 添加打印
    /*
    printf("track_cnt size is %lu \n",track_cnt.size());
    if (track_cnt.size() > 0) {
        printf("before add one, track_cnt[0] is %d \n", track_cnt[0]);
    }
    else
        printf("track_cnt size is 0 \n");
    for (auto &n : track_cnt)
        n++;

    if (track_cnt.size() > 0) {
        printf("after add one, track_cnt[0] is %d \n", track_cnt[0]);
    }
    ...
    if(!_img1.empty() && depth_cam)
    {
        ...
        if(!cur_pts.empty())
        {
            ...
            for(size_t i = 0; i < N; i++)
            {
                ...
                p_u = (int) cur_pts[i].x;
                p_v = (int) cur_pts[i].y;
                //取深度图像数值，利用深度图像反算右图像对应特征点坐标
                //float d = 1.15 * rightImg.ptr<unsigned short >(p_v)[p_u]; //old 参数
                float d = rightImg.ptr<unsigned short >(p_v)[p_u]; //6-21 hmh修改
                //cout <<d<<endl;
                if(d > 0 && d < 7000)
                {
                    //cur_un_right_pts[i].x = cur_un_pts[i].x - 100.0 / d;
                    cur_un_right_pts[i].x = cur_un_pts[i].x - 49.8 / d;//6-21 hmh修改
